...
const WinTrades = () => {
  ...
  useEffect(() => {
    if (user?._id) fetchWinTrades(user._id);
  }, [user]);

  const fetchWinTrades = async (userId) => {
    try {
      setLoading(true);
      const response = await tradeAPI.getAllTrades(userId);
      const winTrades = (response.data?.trades || []).filter(
        (t) => t.status === "completed" && Number(t.profitLoss) > 0
      );
      setTrades(winTrades);
    } catch (err) {
      console.error("Failed to fetch win trades:", err);
      setError("Failed to fetch win trades. Please try again later.");
      setTrades([]);
    } finally {
      setLoading(false);
    }
    ...
    <div className="mx-auto max-w-6xl p-6">
      <header className="mb-6">
        ...
        <h1 className="text-2xl font-semibold text-foreground mb-3">
          <TrendingUp className="text-emerald-600" /> Winning Trades
        </h1>
